...
function EventDemo()
	print("------------------------- Event DEMO")
	local event_id = EventManager:Add(Event.GlobalSendProtocol, GlobalSendProtocol)

	RoleExpCtrl:CheckRoleExp()

	-- remove before
	EventManager:Fire(Event.GlobalSendProtocol, "Request RoleExp")
	-- remove later
	EventManager:Remove(event_id)
	EventManager:Fire(Event.GlobalSendProtocol, "Request RoleExp")
end
